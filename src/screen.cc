...
#define deadline_close(deadline)                                              \
...
void
Screen::draw_helpbar (window_coor c)
{
  for (int i = 0; i < c.cols; ++i)
    whelp->_addch (' ');
  action_list list;
  config.getActionList (list);
  wstring help_bar = L" " + list[L"quit"] + L":quit  " + list[L"up"] + L":up  "
                     + list[L"down"] + L":down  " + list[L"out"] + L":out  "
                     + list[L"in"] + L":in  " + list[L"done"] + L":done  "
                     + list[L"addTodo"] + L":add  " + list[L"editTitle"]
                     + L":modify";
  whelp->_move (0, 0);
...
wstring
Screen::date2str (Date &date)
{
  wchar_t str[11];

  if (config.useUSDates ())
    {
      swprintf (str, 11, L"%2d/%2d/%4d", date.month (), date.day (),
                date.year ());
    }
  else
    {
      swprintf (str, 11, L"%2d/%2d/%4d", date.day (), date.month (),
                date.year ());
    }
...
int
Screen::startTitle (int depth)
{
  return depth * 4 + 7;
}

void
Screen::drawTitle (int line, int depth, wstring &title, int startLine)
{
  int lines, cols;
  ...
  const unsigned int title_lines = (title_cols && !(title_cols % line_cols))
                                       ? (title_cols / line_cols) - 1
...
void
Screen::drawTask (int line, int depth, ToDo &t, bool isCursor)
{
  ...
  if (coor.exist[WDEADLINE])
    {
      ...
      if ((!t.done () && deadline_close (deadline))
          || (isCollapse (t) && deadline_close (chinf.deadline)))
        {
...
void
Screen::drawSched (Sched &sched, pToDo cursor)
{
  if (!coor.exist[WSCHEDULE])
    return;
  ...
  for (sched_l::iterator i = sched_list.begin ();
       (i != sched_list.end ()) && (line < coor.coor[WSCHEDULE].lines); i++)
    {
      ...
      if ((*i)->sched () != last)
        {
          if (line + 2 > coor.coor[WSCHEDULE].lines)
            break;
          last = (*i)->sched ();
      ...
      int printed_chars
          = wschedule->_addstr (title, coor.coor[WSCHEDULE].cols - 4);
...
Editor::return_t
Screen::editTitle (int line, int depth, bool haveChild, wstring &str,
                   int cursorPos)
{
  ...
  if (save == Editor::NOT_SAVED)
    {
      ...
      for (int i
           = startTitle (depth) + (str.length () - 1 % coor.coor[WTREE].cols);
...
Editor::return_t
Screen::editDeadline (int line, Date &deadline, bool done, int cursorPos)
{
  if (!coor.exist[WDEADLINE])
...
Editor::return_t
Screen::editSched (Date &s, int cursorPos)
{
  if (!coor.exist[WSCHEDULE])
    return Editor::NOT_SAVED;
...
Editor::return_t
Screen::setPriority (int line, int &priority)
{
  if (!coor.exist[WPRIORITY])
    return Editor::NOT_SAVED;
...
Editor::return_t
Screen::setCategory (int line, wstring &category, int cursorPos)
{
  if (!coor.exist[WCATEGORY])
    return Editor::NOT_SAVED;
  ...
  save
      = categoryEditor.edit (*wcategory, line, 0, config.getCategoryLength ());
...
int
Screen::taskLines (int depth, ToDo &t)
{
  unsigned int title_cols = wcswidth (t.getTitle ().c_str (), string::npos);
  if (title_cols == (unsigned int)-1) // nonprintable wide character occurs
                                      // among these characters
    title_cols = 0;
  int title_lines
      = (title_cols / (coor.coor[WTREE].cols - startTitle (depth))) + 1;

  if (title_cols
      && !(title_cols % (coor.coor[WTREE].cols - startTitle (depth))))
    title_lines -= 1;
  return title_lines;
}

Editor::return_t
Screen::searchText (wstring &pattern, int cursorPos)
{
  if (!coor.exist[WINFO])
    return Editor::NOT_SAVED;
  ...
  save = searchEditor.edit (*winfo, 0, 1, PERCENT_COL - 2);
...
Editor::return_t
Screen::cmd (wstring &command, int cursorPos)
{
  if (!coor.exist[WINFO])
    return Editor::NOT_SAVED;
  ...
  save = cmdEditor.edit (*winfo, 0, 1, PERCENT_COL - 2);
...
void
Screen::infoPercent (int percent)
{
  if (!coor.exist[WINFO])
    return;
...
#define draw_help()                                                           \
  ...
  while (0)
void
Screen::helpPopUp (wstring str[], int len)
{
  int lines, cols;
  getmaxyx (stdscr, lines, cols);
  ...
  for (vector<Window *>::iterator i = pipes.begin (); i != pipes.end (); i++)
    {
      (*i)->_redraw ();
      (*i)->_refresh ();
    }
