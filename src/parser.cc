...
bool
Parser::parse (ToDo &todo, Sched &sched)
{
  wchar_t ch;
...
void
Parser::ptag (iToDo &iterator, Sched &sched)
{
...
Parser::patt (iToDo &iterator)
{
  ...
  if (L"done" == name)
    ...
    {
      if (L"\"yes\"" == data)
        iterator->getCollapse () = true;
      else
        iterator->getCollapse () = false;
    }
}
  ...
  for (; !((*i).end ()); ++(*i))
    {
      ...
      if ((*i)->priority ())
        {
          putTabs ((*i).depth () + 1);
          file << "<priority>" << (*i)->priority () << "</priority>" << endl;
        }
      if (!(*i)->getCategories ().empty ())
        {
          for (set<wstring>::iterator it = (*i)->getCategories ().begin ();
            ...
            {
              putTabs ((*i).depth () + 1);
              file << "<category>" << *it << "</category>" << endl;
            }
        }
      if ((*i)->getText () != L"")
        {
      ...
      if ((*i)->sched ().valid ())
        {
          ...
          file << "<year>" << str << "</year>" << endl;
          putTabs ((*i).depth () + 2);
          file << "<position>" << (*i)->schedPosition () << "</position>"
               << endl;
          putTabs ((*i).depth () + 1);
          file << "</scheduled>" << endl;
        }
