...
Config::Config ()
    : key_comb (NULL), collapse (false), hide_done (false),
      hide_percent (false), visual_tree (false), bold_parent (true),
      loop_move (false), days_warn_deadline (7), us_dates (false),
      old_sched (true)
{
  ...
  while (!file.eof ())
    {
      ...
      if ('@' == option[0])
        getOutContextOption (option, value);
      else
        switch (context)
          {
          case C_GENERAL:
...
void
Config::getOutContextOption (wstring &option, wstring &value)
{
  if (L"@include" == option)
    {
      char path[256];
...
void
Config::getGeneralOption (wstring &option, wstring &value)
{
...
void
Config::insertKeyMap (key_map &k, wstring action, wstring keys)
{
...
bool
Config::getAction (wchar_t key, wstring &action)
{
  if (key_comb)
    {
...
void
Config::getActionList (action_list &list)
{
  list = action_keys;
}
...
void
Config::getThemeOption (wstring &option, wstring &value)
{
  if (L"row" == option)
    {
      getThemeRow (value);
...
void
Config::getThemeRow (wstring &value)
{
  ...
  while (i != wstring::npos)
    {
      ...
      if (i == wstring::npos)
        ...
        n = i - j;
      getThemeWindow (value.substr (j, n), row.windows[win]);
      win++;
    }
  row.num_windows = win;
...
void
Config::getThemeWindow (wstring fmt, theme_window &w)
{
  wstring::size_type i;
...
void
Config::getThemeTree (wstring &value)
{
...
void
Config::getThemeCategoryLength (wstring &value)
{
  char num[5];
  wcstombs (num, value.c_str (), 5);
...
void
Config::getThemeColors (wstring &option, wstring &value)
{
...
short int
Config::getThemeColor (wstring color)
{
...
bool
Config::getHidePercent ()
{
  return hide_percent;
}

bool
Config::getVisualTree ()
{
  return visual_tree;
}

bool
Config::getBoldParent ()
{
  return bold_parent;
}
...
int
Config::getCategoryLength ()
{
  return category_length;
}

void
Config::genWindowCoor (int lines, int cols, windows_defs &coor)
{
...
bool
Config::genWindowHeights (int lines, int height[])
{
  int sum_height;
  int undefined_row;
...
bool
Config::genWindowWidths (int row_index, int cols, windows_defs &coor,
                         int width[])
{
  int top_win;
  int undefined_col = -1;
...
bool
Config::genWindowTree (windows_defs &coor, int height, int x, int y)
{
  int x_tree;
...
bool
Config::_genWindowCoor (int lines, int cols, windows_defs &coor)
{
  int height[MAX_THEME_ROWS];
  int y = 0, x = 0;

  /* check if there is enought rows */
  if (lines < row_index + 8)
    return false;
  ...
  if (!genWindowHeights (lines, height))
    return false;
  ...
  for (int i = 0; i < row_index; ++i)
    {
      ...
      /* calculate width */
      if (!genWindowWidths (i, cols, coor, width))
        return false;
      ...
      for (int j = 0; j < row.num_windows; ++j)
        {
          ...
          switch (win.window)
            ...
            case WSCHEDULE:
              ...
              break;
            case WTREE:
              if (!genWindowTree (coor, height[i], x, y))
                return false;
              break;
            case WNULL:
...
void
Config::getColorList (color_t *color_list[], short int &length)
{
  *color_list = colors;
  length = num_colors;
}
...
void
Config::getColorPair (short int win, short int &foreground,
