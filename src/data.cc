...
wstring
ToDo::getCategoriesStr ()
{
  wstring cat (L"");
  for (set<wstring>::iterator it = _category.begin (); it != _category.end ();
       it++)
    {
      cat += (*it);
      if (it != --(_category.end ()))
...
void
ToDo::addCategory (const wstring &c)
{
  _category.insert (c);
  if (c != NONE_CATEGORY)
    categories.insert (c);
...
void
ToDo::setCategories (set<wstring> &c)
{
  _category = c;
...
void
ToDo::setCategoriesStr (wstring &c)
{
  _category.clear ();
  if (c.empty ())
    return;
      ...
      for (j = 0; !(i.end ()); ++i, ++j)
        {
          ...
          if (aux.deadline.valid ()
              && ((aux.deadline < chinf.deadline) || !chinf.deadline.valid ()))
...
iToDo::end ()
{
  return ((parent->childs.end () == cursor)
          && (parent->childs.end ()-- == cursor));
}
