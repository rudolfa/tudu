...
#define version()                                                             \
  ...
  cout << "TuDu Copyright (C) 2007-2019 Ruben Pollan Bella "                  \
          "<meskio@sindominio.net>"                                           \
       << endl;                                                               \
  cout << "TuDu comes with ABSOLUTELY NO WARRANTY; for details type `tudu "   \
          "-vv'"                                                              \
       << endl;                                                               \
  cout << "This is free software; you are welcome to redistribute it"         \
       << endl;                                                               \
  cout << "under certain conditions. Type `tudu -vv' for details." << endl;

#define copyright()                                                           \
  cout << VERSION_STR << endl;                                                \
  cout << "Copyright (C) 2007-2015 Ruben Pollan Bella "                       \
          "<meskio@sindominio.net>"                                           \
       << endl                                                                \
       << endl;                                                               \
  cout << "TuDu is free software; you can redistribute it and/or modify"      \
       << endl;                                                               \
  cout << "it under the terms of the GNU General Public License as "          \
          "published by"                                                      \
       << endl;                                                               \
  ...
  cout << "You should have received a copy of the GNU General Public License" \
       << endl;                                                               \
  cout << "along with this program.  If not, see "                            \
...
lock_ask ()
{
  ...
  cout << "    different instances of the same file when making changes."
       << endl;
...
main (int argc, char **argv, char *env[])
{
  ...
  Config config;
  bool configErr = !config.load (
      CONFIG_FILE); // the error will be displayed after check args
  ...
  for (i = 1; i < argc; ++i)
    {
      ...
      else if (!strncmp ("--help", argv[i], 6) || !strncmp ("-h", argv[i], 2))
        {
          usage ();
          return 0;
        }
  ...
  if (configErr)
    {
      fprintf (stderr,
               "Err: Global config does not exist. The config should be %s\n",
               CONFIG_FILE);
      exit (1);
    }
  ...
  if (!p.parse (node, sched))
    {
      ...
      if (!welcome.parse (node, sched))
        {
          fprintf (stderr,
                   "Err: Welcome file does not exist. It should be %s\n",
                   WELCOME_FILE);
          unlink (file_lock);
          exit (1);
        }
