...
void
Cmd::get_interface (Interface *i)
{
  interface = i;
}

bool
Cmd::cmd (wstring command)
{
  ...
  else if (L"showall" == com)
    {
      showall (params);
      return true;
    }
  else if (L"showonly" == com)
    {
      showonly (params);
      return true;
    }
...
void
Cmd::hide (vector<wstring> &params)
{
...
void
Cmd::show (vector<wstring> &params)
{
  for (vector<wstring>::iterator p = params.begin (); p != params.end (); p++)
    {
...
void
Cmd::showall (vector<wstring> &params)
{
  interface->hidden_categories.clear ();
}

void
Cmd::showonly (vector<wstring> &params)
{
  /* add all the categories to the hidden list */
  interface->hidden_categories.insert (L"");
  for (set<wstring>::iterator c = categories.begin (); c != categories.end ();
       c++)
    {
...
void
Cmd::help (vector<wstring> &params)
{
  char *argv[3];
  char argv0[] = "man";
  char argv1[] = "tudu";
