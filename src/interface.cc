...
Interface::Interface (Screen &s, iToDo &t, Sched &sch, Config &c, Writer &w,
                      Cmd &com)
    : screen (s), cursor (t), sched (sch), config (c), writer (w),
      copied (NULL), cmd (com)
{
...
Interface::~Interface () { delete copied; }
  ...
  while (true)
    {
      if (get_wch (&key) == KEY_CODE_YES)
      ...
      else if (config.getAction (key, action))
        {
          ...
          if (L"up" == action)
            up ();
          if (L"move_down" == action)
            move_down ();
          if (L"move_up" == action)
            move_up ();
          if (L"delete" == action)
            del ();
          if (L"delDeadline" == action)
            delDeadline ();
          if (L"delPriority" == action)
            delPriority ();
          if (L"delSched" == action)
            delSched ();
          if (L"paste" == action)
            paste ();
          if (L"pasteUp" == action)
            pasteUp ();
          if (L"pasteChild" == action)
            pasteChild ();
          if (L"done" == action)
            done ();
          if (L"addTodo" == action)
            addLine ();
          if (L"addTodoUp" == action)
            addLineUp ();
          if (L"editTitle" == action)
            modifyLine ();
          if (L"editDeadline" == action)
            editDeadline ();
          if (L"setPriority" == action)
            setPriority ();
          if (L"setCategory" == action)
            setCategory ();
          if (L"editText" == action)
            editText ();
          if (L"editSched" == action)
            editSched ();
          if (L"schedUp" == action)
            schedUp ();
          if (L"schedDown" == action)
            schedDown ();
          if (L"downText" == action)
            downText ();
          if (L"upText" == action)
            upText ();
          if (L"collapse" == action)
            collapse ();
          if (L"hideDone" == action)
            hide_done ();
          if (L"search" == action)
            search ();
          if (L"searchText" == action)
            searchTextCmd ();
          if (L"searchNext" == action)
            search_next ();
          if (L"searchPrev" == action)
            search_prev ();
          if (L"cmd" == action)
            command_line ();
          if (L"sortByTitle" == action)
            sortByTitle ();
          if (L"sortByDone" == action)
            sortByDone ();
          if (L"sortByDeadline" == action)
            sortByDeadline ();
          if (L"sortByPriority" == action)
            sortByPriority ();
          if (L"sortByCategory" == action)
            sortByCategory ();
          if (L"sortByUser" == action)
            sortByUser ();
          if (L"sortRevTitle" == action)
            sortRevTitle ();
          if (L"sortRevDone" == action)
            sortRevDone ();
          if (L"sortRevDeadline" == action)
            sortRevDeadline ();
          if (L"sortRevPriority" == action)
            sortRevPriority ();
          if (L"sortRevCategory" == action)
            sortRevCategory ();
          if (L"sortRevUser" == action)
            sortRevUser ();
          if (L"save" == action)
            save ();
          if (L"help" == action)
...
Interface::drawTodo ()
{
  ...
  if (cursor_line < 0)
    {
      screen.drawTitle (cursor_line, cursor.depth (), cursor->getTitle (),
                        -cursor_line);
      next ();
    }
...
Interface::drawCursor ()
{
  if (fitCursor ())
    ...
    {
      screen.drawTask (cursor_line, cursor.depth (), *cursor, true);
      screen.drawText (cursor->getText ());
      screen.drawSched (sched, &(*cursor));
    }
}
  ...
  if (cursor.depth ())
  ...
  else if (hidden_categories.count (L""))
    {
      ...
      for (cat = categories.begin ();
           (cat != categories.end ()) && hidden_categories.count (*cat); cat++)
        ;
      if (cat != categories.end ())
        cursor->addCategory (*cat);
      else
        cursor->addCategory (NONE_CATEGORY);
    }
}
...
bool
Interface::editLine (wstring &str)
{
  ...
  save = screen.editTitle (cursor_line, cursor.depth (), cursor->haveChild (),
                           str, str.length ());
...
void
Interface::editDeadline ()
{
  screen.infoMsg (
      "Editing deadline. Press ENTER to save or ESC to abort edit");
...
{
  screen.infoMsg (
      "Editing priority. Press ENTER to save or ESC to abort edit");
...
{
  screen.infoMsg ("Editing categories. Separate multiple categories with ','. "
                  "Press ENTER to save or ESC to abort edit");
  ...
  if (res)
    {
      iToDo aux = cursor;
      while (aux.out ())
        aux->actCollapse () = false;
      aux = hit;
      while (hit.out ())
        hit->actCollapse () = true;
      ...
      if (cmd.cmd (command))
        {
          ...
          if ((cursor->getCategories ().count (NONE_CATEGORY))
              && cursor->getTitle ().empty ())
            del ();
          drawTodo ();
        }
    }
...
Interface::help ()
{
  ...
  str[i++]
      = L"  " + list[L"addTodoUp"] + L"\tadd a new task above the cursor\n";
  str[i++] = L"  " + list[L"editTitle"] + L"\tedit task's title\n";
  str[i++] = L"  " + list[L"editText"]
             + L"\tadd/edit task's long description text\n";
  ...
  str[i++]
      = L"  " + list[L"editSched"] + L"\tadd/edit task's scheduled date\n";
  str[i++] = L"\n";
  str[i++] = L"  NAVIGATION\n";
  ...
  str[i++]
      = L"  " + list[L"hideDone"] + L"\thide/unhide tasks marked as done\n";
  str[i++] = L"  " + list[L"move_down"] + L"\tmove a task downwards\n";
  str[i++] = L"  " + list[L"move_up"] + L"\tmove a task upwards\n";
  str[i++]
      = L"  " + list[L"schedUp"] + L"\tmove up the task on the scheduler\n";
  str[i++] = L"  " + list[L"schedDown"]
             + L"\tmove down the task on the scheduler\n";
  ...
  str[i++]
      = L"  " + list[L"paste"] + L"\tpaste last deletion below the cursor\n";
  str[i++]
      = L"  " + list[L"pasteUp"] + L"\tpaste last deletion above the cursor\n";
  str[i++]
      = L"  " + list[L"pasteChild"] + L"\tpaste last deletion as a subtask\n";
  str[i++] = L"\n";
  str[i++] = L"  COMMANDS\n";
