...
Editor::return_t
Editor::edit (Window &win, int begin_y, int begin_x, int ncols)
{
  window = &win;
  y = begin_y;
  x = begin_x;
...
LineEditor::updateText ()
{
  ...
  int cursor_cols
      = cursor > offset
            ? wcswidth (text.substr (offset, cursor - offset).c_str (), cols)
            : 0;
  ...
  for (int line = 0; line <= textLines; line++)
    {
      ...
      if (!cursor_found && ((line == textLines) || (i + num_cols > cursor)))
        {
          cursor_line = line;
          cursor_col = 0;
 ...
 */
Editor::return_t
CategoryEditor::edit (Window &win, int begin_y, int begin_x, int ncols)
{
...
bool
CategoryEditor::cmp (unsigned int idx, wstring str)
{
  return text.compare (idx, length, str, 0, length);
}

void
CategoryEditor::tab () /* do completion */
{
  unsigned int j, i = 0;
  ...
  if ((cursor == (int)j) && (search != categories.end ()) && (cat == *search))
    {
      search++;
      if ((search != categories.end ()) && (!cmp (i, *search)))
        {
          text = pre + *search + pos;
  ...
  /* if it is the first time */
  else
    {
      ...
      for (search = categories.begin ();
           (search != categories.end ()) && (cmp (i, *search)); search++)
        ;
      if ((search != categories.end ()) && (!cmp (i, *search)))
        {
          text = pre + *search + pos;
...
bool
CmdEditor::cmp (wstring str)
{
  return text.compare (0, length, str, 0, length);
}

void
CmdEditor::tab () /* do completion */
{
  ...
  for (begin = 0, end = text.find (' ', 0);
       (wstring::npos != end) && (cursor > (int)end);
...
void
CmdEditor::command_completion (wstring &com)
{
  /* if it is not the first time */
  if ((param == 0) && (com_search != commands.end ())
      && (com == com_search->first))
    {
      com_search++;
      if ((com_search != commands.end ()) && (!cmp (com_search->first)))
        {
          com = com_search->first;
          cursor = com.length ();
        }
  ...
  /* if it is the first time */
  else
    {
      ...
      for (com_search = commands.begin ();
           (com_search != commands.end ()) && (cmp (com_search->first));
           com_search++)
        ;
      if ((com_search != commands.end ()) && (!cmp (com_search->first)))
        {
...
void
CmdEditor::category_completion (wstring &cat, int num_param)
{

  /* if it is no the first time */
  if ((param == num_param) && (search != categories.end ())
      && (cat == *search))
    {
      search++;
      if ((search != categories.end ()) && (!param_cmp (*search)))
        {
          cat = *search;
        }
      else
  ...
  /* if it is the first time */
  else
    {
      length = cat.length ();
      for (search = categories.begin ();
           (search != categories.end ()) && (param_cmp (*search)); search++)
        ;
      if ((search != categories.end ()) && (!param_cmp (*search)))
        {
          cat = *search;
 ...
 */
Editor::return_t
DateEditor::edit (Window &win, int begin_y, int begin_x)
{
  return Editor::edit (win, begin_y, begin_x, 11);
}

void
DateEditor::updateText ()
{
  if (cursor >= (int)cols)
    cursor = cols - 1;
 ...
 */
Editor::return_t
PriorityEditor::edit (Window &win, int begin_y, int begin_x)
{
  return Editor::edit (win, begin_y, begin_x, 1);
}

void
PriorityEditor::updateText ()
{
  cursor = 0;
  window->_addstr (y, x, text);
  window->_move (y, x);
