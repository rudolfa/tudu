...
wstring
Text::_getStr (list<wstring>::iterator begin, int length)
{
  int rows = 0;
  wstring s = L"";
...
Text::tab ()
{
  ...
  if (TAB_SPACES > cursor_x)
    ++cursor_y;
}

wostream &
operator<< (wostream &os, Text &t)
{
  os << t.getStr ();
  return os;
}
